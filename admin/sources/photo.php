...
function save_photo()
{
	...
	if(isset($_FILES['file-taptin']))
	{
		...
		if($taptin = $func->uploadImage("file-taptin", $config['photo']['man_photo'][$type]['img_type_video'],UPLOAD_FILE,$file_name))
		{
			$data['taptin'] = $taptin;
			$row = $d->rawQueryOne("select id, taptin from #_photo where type = ? limit 0,1",array($type));
	...
	else
	{
		...
		if($numberPhoto && $dataMultiTemp)
		{
			for($i=0;$i<count($dataMultiTemp);$i++)
			{
				...
				if(isset($data['taptin']) && $data['taptin']!=''){
					$dataMulti['taptin']=$data['taptin'];
					if(
						(isset($dataMulti['tenvi']) && $dataMulti['tenvi'] != '') || 
						(isset($dataMulti['link']) && $dataMulti['link'] != '') || 
						(isset($dataMulti['taptin']) && $dataMulti['taptin'] != '') || 
						(isset($dataMulti['link_video']) && $dataMulti['link_video'] != '')
