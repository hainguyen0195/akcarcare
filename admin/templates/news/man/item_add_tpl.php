...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                <div class="row">
                     <?php if(isset($config['news'][$type]['conso']) && $config['news'][$type]['conso'] == true) { ?>
                        <div class="form-group  col-md-12 col-sm-12">
                            <label for="text">Con số :</label>
                            <input type="text" class="form-control" name="data[text]" id="text" placeholder="Con số" value="<?= isset($item['text'])?$item['text']:'' ?>">
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['sao']) && $config['news'][$type]['sao'] == true) { ?>
                        <div class="form-group  col-md-12 col-sm-12">
                            <label for="sao">Số sao :</label>
                            <input type="text" class="form-control" name="data[sao]" id="sao" placeholder="Số sao" value="<?= isset($item['sao'])?$item['sao']:'' ?>">
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['diachi']) && $config['news'][$type]['diachi'] == true) { ?>
                        <div class="form-group  col-md-12 col-sm-12">
                            <label for="text">Địa chỉ :</label>
                            <input type="text" class="form-control" name="data[text]" id="text" placeholder="Địa chỉ" value="<?= isset($item['text'])?$item['text']:'' ?>">
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['text']) && $config['news'][$type]['text'] == true) { ?>
                        <div class="form-group  col-md-12 col-sm-12">
                            <label for="text">Text :</label>
                            <input type="text" class="form-control" name="data[text]" id="text" placeholder="Text" value="<?= isset($item['text'])?$item['text']:'' ?>">
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['chucvu']) && $config['news'][$type]['chucvu'] == true) { ?>
                        <div class="form-group  col-md-12 col-sm-12">
                            <label for="text">Chức vụ :</label>
                            <input type="text" class="form-control" name="data[text]" id="text" placeholder="Chức vụ" value="<?= isset($item['text'])?$item['text']:'' ?>">
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['iframe']) && $config['news'][$type]['iframe'] == true) { ?>
                        <div class="form-group col-md-12 col-sm-12">
                            <label for="iframe">Iframe bản đồ :</label>
                            <textarea class="form-control for-seo " name="data[iframe]" id="iframe" rows="5" placeholder="Iframe bản đồ "><?=htmlspecialchars_decode(@$item['iframe'])?></textarea>
                        </div>
                    <?php } ?>
                    <?php  if(isset($config['news'][$type]['info_more']) && $config['news'][$type]['info_more']!='' ) { ?>
                        <?php  foreach($config['news'][$type]['info_more'] as $key_ifm => $value_ifm) {  ?>
                         <div class="form-group col-md-12 col-sm-12">
