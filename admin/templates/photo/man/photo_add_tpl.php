		...
		<?php for($i=0;$i<$numberPhoto;$i++) { $stt = $i+1; ?>
			<div class="card card-primary card-outline text-sm">
				...
				<div class="card-body">
					...
					<?php } ?>
					<?php if(isset($config['photo']['man_photo'][$type]['upvideo']) && $config['photo']['man_photo'][$type]['upvideo'] == true) { ?>
						<div class="form-group">
							...
							<strong class="d-block mt-2 mb-2 text-sm"><?php echo $config['photo']['man_photo'][$type]['img_type_video']; ?></strong>
							<div class="custom-file my-custom-file d-none">
								<input type="file" class="custom-file-input" name="file-taptin" id="file-taptin">
								<label class="custom-file-label" for="file-taptin">Chọn video</label>
							</div>
							<?php if(isset($item['taptin']) && $item['taptin'] != '') { ?>
								<a class="btn btn-sm bg-gradient-primary text-white d-inline-block align-middle p-2 rounded mb-1" href="<?=UPLOAD_FILE.@$item['taptin']?>" title="Download tập tin hiện tại"><i class="fas fa-download mr-2"></i>Download video hiện tại</a>
							<?php } ?>
						</div>
					<?php } ?>
					<?php if(isset($config['photo']['man_photo'][$type]['video_photo']) && $config['photo']['man_photo'][$type]['video_photo'] == true) { ?>
						<div class="form-group">
							<label for="link_video<?=$i?>">Link video youtube:</label>
